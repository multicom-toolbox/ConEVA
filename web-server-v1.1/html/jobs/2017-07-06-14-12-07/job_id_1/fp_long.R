png("fp_long.png", width=2500, height=1400, res=300)
...
for(src in sources){
	...
	if(start_flag == 1){
		plot(selected, col=src_id, type="l", lwd = 2, xaxt='n', ylim=y_range, ylab="Mean FP Error", xlab="Top Ranked Contacts")
		start_flag = 0
	}
	else{
...
if (length(sources) > 1){
	legend("bottomright", inset=c(-0.4,0), legend = sources, cex=1.0, lwd = 3, col = seq(1, length(sources), by = 1), lty = seq(1, length(sources), by = 1))
}
