<?php include("header.php");?>
	<form method="post" onsubmit="return validateForm()" action="http://iris.rnet.missouri.edu/cgi-bin/coneva/main_v2.0.cgi" enctype="multipart/form-data">
...
(2)Paste a http link, for instance, http://iris.rnet.missouri.edu/coneva/download/test/1aapA.rr"></textarea></td>
		</tr>
		<tr id="tr_rr_upload">
			<td></td>
...
(2)Paste contents of PDB file"></textarea></td>
	...
	-->
		...
		<tr>
			...
			<u><a onclick="evaluate_rr('metapsicov', '1a3aA', 'http://iris.rnet.missouri.edu/coneva/download/web-server-examples/1a3aA.zip', 'http://iris.rnet.missouri.edu/coneva/download/web-server-examples/1a3aA.pdb')">Load-1a3aA</a></u>
			&nbsp; &nbsp;<u><a onclick="evaluate_rr('casprr', 'T0763-D1', 'http://iris.rnet.missouri.edu/coneva/download/web-server-examples/T0763.zip', 'http://iris.rnet.missouri.edu/coneva/download/web-server-examples/T0763-D1.pdb')">Load-T0763-D1</a></u>
			...
			<?php
				...
				if ($handle) {
					while (($buffer = fgets($handle, 4096)) !== false) {
						$id = rtrim($buffer);
						$rrurl = "http://iris.rnet.missouri.edu/coneva/preloaded/metapsicov/merged_n_zipped/$id.zip";
						$nativeurl = "http://iris.rnet.missouri.edu/coneva/preloaded/metapsicov/pdb_frg_supp/$id.pdb";
		...
		<tr><td style="text-align:left;">
			...
			<?php
				...
				if ($handle) {
					while (($buffer = fgets($handle, 4096)) !== false) {
						$id = rtrim($buffer);
						$rrurl = "http://iris.rnet.missouri.edu/coneva/preloaded/metapsicov/casp_format/psicov/$id.contacts";
						$nativeurl = "http://iris.rnet.missouri.edu/coneva/preloaded/metapsicov/pdb_frg_supp/$id.pdb";
			...
			<?php
				...
				if ($handle) {
					while (($buffer = fgets($handle, 4096)) !== false) {
						...
						$rrurl = "http://iris.rnet.missouri.edu/coneva/preloaded/evfold/input/$newpair[0].rr";
						$nativeurl = "http://iris.rnet.missouri.edu/coneva/preloaded/evfold/native/$newpair[1]_filtered.pdb";
		...
		<tr><td style="text-align:left;">
			...
			<?php
				...
				if ($handle) {
					while (($buffer = fgets($handle, 4096)) !== false) {
						$id = rtrim($buffer);
						$rrurl = "http://iris.rnet.missouri.edu/coneva/preloaded/casp/11/RR/predictions/$id/$id.zip";
						$nativeurl = "http://iris.rnet.missouri.edu/coneva/preloaded/casp/11/RR/native/$id.pdb";
		...
		<tr><td style="text-align:left;">
			...
			<?php
				...
				if ($handle) {
					while (($buffer = fgets($handle, 4096)) !== false) {
						$id = rtrim($buffer);
						$rrurl = "http://iris.rnet.missouri.edu/coneva/preloaded/casp/11/RR/predictions/$id/${id}RR038_1";
						$nativeurl = "http://iris.rnet.missouri.edu/coneva/preloaded/casp/11/RR/native/$id.pdb";
		...
		<tr><td style="text-align:left;">
			...
			<?php
				...
				if ($handle) {
					while (($buffer = fgets($handle, 4096)) !== false) {
						$id = rtrim($buffer);
						$rrurl = "http://iris.rnet.missouri.edu/coneva/preloaded/casp/11/RR_selected/zipped/$id.zip";
						$nativeurl = "http://iris.rnet.missouri.edu/coneva/preloaded/casp/11/RR/native/$id.pdb";
						echo "<a style=\"text-decoration:none;\" onclick=\"evaluate_rr('casprr', '$id', '$rrurl', '$nativeurl')\">$id</a> \n";
					}
					if (!feof($handle)) {
		...
		<tr><td style="text-align:left;">
			...
			<?php
				...
				if ($handle) {
					while (($buffer = fgets($handle, 4096)) !== false) {
						$id = rtrim($buffer);
						$nativeurl = "http://iris.rnet.missouri.edu/coneva/preloaded/casp/10/native_domains/$id.pdb";
		...
		<tr><td style="text-align:left;">
			...
			<?php
				...
				if ($handle) {
					while (($buffer = fgets($handle, 4096)) !== false) {
						$id = rtrim($buffer);
						$nativeurl = "http://iris.rnet.missouri.edu/coneva/preloaded/casp/11/native_domains/$id.pdb";
						echo "<a style=\"text-decoration:none;\" onclick=\"native_analysis('casp', '$id', '$nativeurl')\">$id</a> \n";
					}
					if (!feof($handle)) {
						echo "Error: unexpected fgets() fail\n";
			...
			<?php
				...
				if ($handle) {
					while (($buffer = fgets($handle, 4096)) !== false) {
						$id = rtrim($buffer);
						$rrurl = "http://iris.rnet.missouri.edu/coneva/preloaded/dncon_vs_metaPSICOV/$id.zip";
						$nativeurl = "http://iris.rnet.missouri.edu/coneva/preloaded/casp/11/RR/native/$id.pdb";
						echo "<a style=\"text-decoration:none;\" onclick=\"evaluate_rr('casprr', '$id', '$rrurl', '$nativeurl')\">$id</a> \n";
					}
					if (!feof($handle)) {
