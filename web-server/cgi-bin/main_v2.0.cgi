...
use File::Basename;
use lib '/srv/http/cgi-bin/conassess/Roman-1.24/lib';
use Roman;
use Time::Piece;

$CGI::POST_MAX = 1024 * 10000; # 10MB
...
my $long_range_max   = $query -> param("lrmax");         # 10000;
...
my $APP_ROOT = "/srv/http/cgi-bin/conassess";
...
open HEADER, "/srv/http/html/coneva/header.php"; 
while(<HEADER>){
	print $_;
}
close HEADER;
...
my $file_history = "/srv/http/html/coneva/jobs/history.log";
system("rm -f /srv/http/html/coneva/jobs/clientmap");
my $file_clientmap = "/srv/http/html/coneva/jobs/map";
my %client_map;
...
my $dir_client = "/srv/http/html/coneva/jobs/$remote_addr";
...
my $web_root = "http://cactus.rnet.missouri.edu/coneva/jobs/$remote_addr/$job_id";
...
if ($native_pdb){
	...
	$native_rr = "native.rr";
	%true_contacts = rrfile_to_r1r2hash($native_rr, $min_seq_sep, 100000);
...
sub print_contact_counts{
	...
	foreach my $src (sort keys %sources){
		...
		my %rr = rr_rows_ordered_in_hash($input_rr, 100000, $src, "all");
		my $ignored_rr_cnt = 0;
		if (-f "./not-in-native/not_in_native_$src.rr"){
			my %rr_ignored = rr_rows_ordered_in_hash("./not-in-native/not_in_native_$src.rr", 100000, $src, "all") ;
		...
		my %sr  = rr_rows_ordered_in_hash("temp.rr", 100000, "all", "short");
		my %mr  = rr_rows_ordered_in_hash("temp.rr", 100000, "all", "medium");
		my %lr  = rr_rows_ordered_in_hash("temp.rr", 100000, "all", "long");
		my %all = rr_rows_ordered_in_hash("temp.rr", 100000, "all", "all");
		my $srcnt;
		my $mrcnt;
		my $lrcnt;
		my $alcnt;
...
sub print_jaccard_similarity{
	...
	foreach my $top (sort {$top_count_order{$a} <=> $top_count_order{$b}} keys %top_count_order){
		...
		print2file("$top.R", "if (!require(\"gplots\")) {");
		print2file("$top.R", "  install.packages(\"gplots\", dependencies = TRUE)");
		print2file("$top.R", "  library(gplots)");
		print2file("$top.R", "}");
		print2file("$top.R", "if (!require(\"RColorBrewer\")) {");
		print2file("$top.R", "  install.packages(\"RColorBrewer\", dependencies = TRUE)");
		print2file("$top.R", "  library(RColorBrewer)");
		print2file("$top.R", "}");
		print2file("$top.R", "png(\"$top.png\", width=2000, height=2000, res=300)");
		print2file("$top.R", "table_jaccard = read.table(file=\"mat_top${top}.txt\")");
		print2file("$top.R", "jacc_matrix = data.matrix(table_jaccard)");
		print2file("$top.R", "heatmap.2(jacc_matrix, tracecol = NA, symm = T, dendrogram = \"col\", Colv = T, scale = 'none', col=colorRampPalette(brewer.pal(8,\"YlOrRd\"))(length(jacc_matrix)), key.ylab = NA, key.xlab = \"Jaccard Similarity\", lmat = rbind(c(3,4),c(1,2)), lwid = c(4,1), lhei = c(1,4), density.info=\"none\", margins = c(12, 12), srtCol=90, main=\"$id\")");
		system_cmd("R --vanilla < $top.R &>> $file_log");
		$i++;
...
sub print_chord_diagram{
	...
	foreach my $top (sort {$top_count_order{$a} <=> $top_count_order{$b}} keys %top_count_order){
		...
		system_cmd("python $APP_ROOT/chord_diagram.py input_top$top.rr top$top.R top$top.png $id &>> $file_log");
		append2log("Plot for Top $top..");
		system_cmd("R --vanilla --slave < top$top.R &>> $file_log");
		$i++;
		last if $i == $MAX_DIAGRAMS + 1;
	}
	print "\n<table class=\"results\">";
...
sub print_roc{
	...
	foreach my $src (sort keys %sources){
		...
		my %rows = rr_rows_ordered_in_hash("input.rr", 100000, $src, $param_cont_type);
		foreach (keys %rows){
			my @C = split /\s+/, $rows{$_};
	...
	system_cmd("R --vanilla --verbose < roc.R &>> $file_log");
...
sub print_auc_pr{
	...
	foreach my $src (sort keys %sources){
		system_cmd("java -jar $APP_ROOT/auc.jar $src.txt list &> auc_pr_$src.log");
		system_cmd("tail -2 auc_pr_$src.log > auc_pr_$src.txt");
	}
	foreach my $src (sort keys %sources){
		my $line = `head -1 auc_pr_$src.txt`;
		chomp $line;
		my @C = split /\s+/, $line;
	...
	system_cmd("R --vanilla --verbose < pr.R &>> $file_log");
...
sub print_MCC{
	...
	print "\n<li><img src=\"http://cactus.rnet.missouri.edu/coneva/mcc-eqn.png\" width=\"325\"/></li>";
	...
	if (defined $param_rr_raw){
		if($param_rr_raw !~ /^http/){
			...
			if($filename =~ /zip/){
				...
				# Check if the files contain contact types of interest
				# This also fixes the error of R1 > R2;
				my %rr = rr_rows_ordered_in_hash($input_rr, 10000, "all", $param_cont_type);
...
sub chk_errors_input_files{
	...
	if (defined $rr_sequence){
		...
		if (-f $native_pdb){
			system_cmd("cp /srv/http/html/coneva/header.php sequence_compare.html");
			...
			system_cmd("cat /srv/http/html/coneva/footer.php >> sequence_compare.html");
		}
		if (-f $input_sec){
...
sub print_footer{
	open FOOTER, "/srv/http/html/coneva/footer.php"; 
	while(<FOOTER>){
		print $_;
	}
	close FOOTER;
...
sub calc_coverage{
	...
	my %rr = %{$rrhash};
	error_exit("Cannot calculate precision! Empty selected contacts") if not scalar keys %rr;
	my %rrpdb = rr_rows_ordered_in_hash($native_rr, 10000, "all", $param_cont_type);
...
sub calc_spread{
	...
	my $file_rr = shift;
	my %predicted = rrfile_to_r1r2hash($file_rr, 1, 10000);
...
sub load_sources{
	my $rr = shift;
	# Contact sources
	my %rr = rr_rows_ordered_in_hash($rr, 100000, "all", "all");
	%sources = ();
	foreach (sort {$a <=> $b} keys %rr){
		my @C = split /\s+/, $rr{$_};
		last if not defined $C[5];
	...
	foreach my $src (sort keys %sources){
		my %rr_selected = rr_rows_ordered_in_hash($input_rr, 10000, $src, $param_cont_type, 0.5);
...
sub print_contact_maps{
	...
	foreach my $top (sort {$top_count{$a} <=> $top_count{$b}} keys %top_count){
		...
		if(-f "$dir_job/$native_pdb"){
			my %rrn = rr_rows_ordered_in_hash("$dir_job/$native_rr", 10000, "all", "all");
...
sub print_native_contact_maps{
	...
	foreach my $select (sort keys %selections){
		system_cmd("rm -f temp.rr");
		print2file("temp.rr", seq_rr($input_rr));
		my %rr = rr_rows_ordered_in_hash($native_rr, 10000, "all", $select);
		foreach my $row (sort {$a <=> $b} keys %rr){
			print2file("temp.rr", $rr{$row});
		}
...
sub print_precision{
	...
	print2file("precision_$param_cont_type.R", "}");
	system_cmd("R --vanilla < precision_$param_cont_type.R &>> $file_log");
...
sub print_coverage{
	...
	print2file("coverage_$param_cont_type.R", "}");
	system_cmd("R --vanilla < coverage_$param_cont_type.R &>> $file_log");
...
sub print_xd{
	...
	print2file("xd_$param_cont_type.R", "}");
	system_cmd("R --vanilla < xd_$param_cont_type.R &>> $file_log");
	append2log("Show Xd"); 
	print "\n<table class=\"results\">";
...
sub print_spread{
	...
	print2file("spread_$param_cont_type.R", "}");
	system_cmd("R --vanilla < spread_$param_cont_type.R &>> $file_log");
...
sub print_FP_assessment{
	...
	print2file("fp_$param_cont_type.R", "}");
	system_cmd("R --vanilla < fp_$param_cont_type.R &>> $file_log");
	append2log("Show Xd"); 
	print "\n<table class=\"results\">";
...
sub plot_contact_map{
	...
	system_cmd("R --vanilla < cmap.R &>> $file_log");
}
